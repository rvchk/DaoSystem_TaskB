// contract.js (обновленная версия с новыми функциями)

const { Contract } = require('fabric-contract-api');

class DaoSystem extends Contract {

  // === Инициализация реестра начальными данными ===
  async InitLedger(ctx) {
      ...
      for (const startup of startups) {
          ...
          const existingStartupAsBytes = await ctx.getState(startup.id);
          if (!existingStartupAsBytes || existingStartupAsBytes.length === 0) {
              console.log(`Создаём начальный стартап: ${startup.id}`);
              await ctx.setState(startup.id, Buffer.from(JSON.stringify(startup)));
  ...
  async recordEvent(ctx, eventType, data) {
      ...
      await ctx.setState(key, Buffer.from(JSON.stringify(event)));

      await ctx.emitEvent('DAOEvent', Buffer.from(JSON.stringify(event)));
  }

  // === Создание нового стартапа ===
  async createStartup(ctx, startupId, ethereumAddress) {
      const startupAsBytes = await ctx.getState(startupId);
      if (startupAsBytes && startupAsBytes.length > 0) {
          throw new Error(`Стартап с ID ${startupId} уже существует`);
      }
      ...
      const startup = {
          ...
          requests: [] // Список заявок на расходы
      };

      await ctx.setState(startupId, Buffer.from(JSON.stringify(startup)));

      await this.recordEvent(ctx, 'STARTUP_CREATED', {
          startupId: startupId,
          ethereumAddress: ethereumAddress
      });

      console.log(`Стартап ${startupId} создан`);
  }
  ...
  async distributeFundsInsideStartup(ctx, startupId, totalAmount) {
      ...
      const marketingShare = 0.10;
      const developmentShare = 0.40;
  ...
  async redistributeFunds(ctx, startupId, fromDepartment, toDepartment, amount) {
      const startupAsBytes = await ctx.getState(startupId);
      if (!startupAsBytes || startupAsBytes.length === 0) {
          throw new Error(`Стартап с ID ${startupId} не найден`);
      }
      ...
      if (fromDepartment !== 'management') {
          throw new Error("Перераспределение может производить только отдел управления");
      }
      ...
      console.log(`Средства в размере ${amount} перераспределены из ${fromDepartment} в ${toDepartment}`);
  }
  ...
  async checkAndCreateRefundRequest(ctx, startupId) {
      ...
      for (const dept of ['marketing', 'development', 'legal']) {
          ...
          if (initialAmount > 0 && currentAmount <= initialAmount * 0.1) {
              ...
              console.log(`Авто-запрос на дофинансирование для ${dept}: ${request.amount}`);
          }
      }
  }

  // === Получение информации о стартапе ===
  async getStartup(ctx, startupId) {
      const startupAsBytes = await ctx.getState(startupId);
      if (!startupAsBytes || startupAsBytes.length === 0) {
          throw new Error(`Стартап с ID ${startupId} не найден`);
      }
      return startupAsBytes.toString();
  }
